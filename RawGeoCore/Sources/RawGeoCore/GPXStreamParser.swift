...
public struct GPXStreamParser: Sendable {
    ...
    private func parse(parser: XMLParser) throws -> GPXDocument {
        ...
        guard parser.parse() else {
            throw GPXParseError.malformedXML(parser.parserError?.localizedDescription ?? "Unknown XML error")
        }
...
private final class ParserDelegate: NSObject, XMLParserDelegate {
    ...
    private func finishCurrentPoint() {
        ...
        guard let timestamp = fractionalDateFormatter.date(from: timestampText)
        ...
        let accuracy = point.horizontalAccuracyText.flatMap(Double.init).flatMap { $0 >= 0 && $0.isFinite ? $0 : nil }
