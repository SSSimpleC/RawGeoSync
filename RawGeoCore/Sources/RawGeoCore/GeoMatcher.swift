...
public struct GeoMatcher: Sendable {
    ...
    private func makeCandidates(
        ...
        if let exact = exactPoints
            .filter({ abs($0.timestamp.timeIntervalSince(photo.captureTimeUTC)) <= configuration.exactToleranceSeconds })
        ...
        let fraction = photo.captureTimeUTC.timeIntervalSince(interval.start.timestamp)
    ...
    private func endpointCandidate(
        ...
        guard abs(photo.captureTimeUTC.timeIntervalSince(point.timestamp))
    ...
    private func sensorAccuracy(for candidate: MatchCandidate) -> SensorAccuracy {
        let values = [candidate.startPoint.horizontalAccuracyMeters,
                      candidate.endPoint?.horizontalAccuracyMeters].compactMap { $0 }
