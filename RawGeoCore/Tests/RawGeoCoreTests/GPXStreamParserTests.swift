...
final class GPXStreamParserTests: XCTestCase {
  ...
  func testCancelledTaskStopsParsing() async {
    let xml =
      "<gpx version=\"1.1\"><trk><trkseg><trkpt lat=\"23\" lon=\"113\"><time>2026-01-01T00:00:00Z</time></trkpt></trkseg></trk></gpx>"
    let task = Task {
      try GPXStreamParser().parse(data: Data(xml.utf8))
    }
    task.cancel()

    do {
      _ = try await task.value
      XCTFail("已取消的解析任务不应完成")
