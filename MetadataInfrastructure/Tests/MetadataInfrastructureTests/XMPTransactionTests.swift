...
struct XMPTransactionTests {
    ...
    func cancellationKeepsSuccesses() async throws {
        ...
        try await coordinator.undo(transactionID: report.transactionID)
        #expect(!FileManager.default.fileExists(atPath: try SidecarURL(for: context.rawFiles[0]).url.path))
    }
    ...
    func undoConflict() async throws {
        ...
        await #expect(throws: MetadataInfrastructureError.transactionCannotBeUndone(report.transactionID)) {
            try await coordinator.undo(transactionID: report.transactionID)
        }
